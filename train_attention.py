...
from model_attention import Encoder, Decoder, Seq2Seq  # attention‑equipped model

def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Seq2Seq transliteration model with attention"
    )
    # data & vocab
    ...
    parser.add_argument("--model_dir",   type=str, default="models_attention",
...
def main():
    ...
    # decoder uses encoder's hidden dimension (doubled if bidirectional)
    enc_hid = args.hid_size * (2 if args.bidirectional else 1)
    decoder = Decoder(
        output_size=len(tgt_vocab),
        embedding_size=args.emb_size,
        enc_hidden_size=enc_hid,
        dec_hidden_size=args.hid_size,
        num_layers=args.num_layers,
        dropout=args.dropout,
        cell_type=args.cell_type
    )
    ...
    # full model
    model = Seq2Seq(encoder, decoder, device, teacher_forcing_ratio=args.teacher_forcing)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        ckpt = os.path.join(args.model_dir, f"attention_ep{ep}.pt")
        torch.save(model.state_dict(), ckpt)

        # save best
        if dev_acc > best_dev_acc:
            best_dev_acc = dev_acc
            torch.save(model.state_dict(), os.path.join(args.model_dir, "best_attention.pt"))
